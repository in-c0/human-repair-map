...
### jrieke-thesis-repro — Rieke J. (2016) "Characterizing the dynamics of a Drosophila motoneuron model via bifurcation analysis" (bachelor thesis; code+data github.com/jrieke/drosophila-dynamics; thesis text github.com/jrieke/bachelor-thesis)
- **Key result / content:** An independent third-party reproduction of the Günay 2015 isopotential model in XPP and NeuroML2, with AUTO continuation in I and in each conductance. The `rheobase/README.md` states the exact staircase protocols and outcomes: from the −12 pA settled state, silence → tonic firing between **I = −1.91 and −1.90 pA** (absolute; 5 s per 0.01 pA step); from tonic firing at −1.80 pA, tonic → silence between **−2.72 and −2.73 pA** (a ≈0.8 pA bistable region). Delay-to-first-spike runs at −1.5 … 50 pA (NeuroML2, dt 0.001 ms). The `.dat` files are Git-LFS pointers (not fetchable here); the README text and notebooks are readable.
- **Usable quantities (Verified: README + notebooks read):** the two transition currents above; the steady-state I–V construction (`I-V-curve/calculation.ipynb`) uses the identical equations and parameters as the XPP file (gKs 50, gKf 24.1, fh 0.95, gNa 100, gNaP 0.8, gleak 6.8, Eleak −55, EK −80, ENa 45).
- **Use in V1:** Level-C numerical anchor for the medium level (reproduced exactly by the port; `experiments/v1_channel/results/gunay2015_reproduction/`).
- **Verified:** yes — cloned https://github.com/jrieke/drosophila-dynamics (README, notebooks read; data are LFS pointers).

### gunay-2015 stimulus-convention note (added after the port)
The paper's text reports for the isopotential model "CV = 0.002 (10 pA and 50 Hz)". The XPP file holds the cell at I = −12 pA and steps to an absolute `Ipulse`. With the port, a step to **0 pA absolute (= +12 pA from the hold)** fires at 49.5 Hz with CV 0.0010, and +10 pA absolute fires at 91.5 Hz with CV 0.0023; the CV is reproduced either way, the rate only under the relative reading. The Methods sentence "The holding current for stimulus protocols started at −6.5 pA" sits in the two-compartment paragraph. The convention is therefore **unresolved from the readable text** and is recorded as such in the reproduction report; it does not affect V1 (whose protocols are stated in absolute currents).
