...
|----|------------|----------------------------------|-------------------|-----------|
| H0 | adaptive physical computation | V0-passed, conditional (H1 ∧ H3/H9 within the synthetic family; falsified when cheap-model error is not sparse). **V1: not separately preregistered; the learned VoC controller was falsified (H1-V1) while adaptive refinement itself dominates uniform levels on the published cell (oracle 0.21 tolerances at 24 % of uniform-fine cost vs 8.0 for the published model; the ensemble-spread trigger 0.17 at 40 %)** — research log 2026-09-15 | V0, V1 | this file; `2026-09-15-v1-final-report.md` |
...
| H9 | value of computation | **V1-passed (precursor, cross-formulation transfer)** (`v1_gunay_main`, `cross_formulation/`, `verdicts_extended.md`): the VoC router's selection precision under the level-B truth (0.80) ≥ 0.8× its precision under A (0.65) and ≥ every written rule's under B (best 0.75) — a ranking that survives the change of formulation; every policy's precision rises under B (larger minimal sets), the margin over the sensitivity rule is narrow (1.12× at equal recall), the two truths disagree on 37 % of labels, the router's error rises 0.72 → 1.24 and its false-safe rate doubles; the hybrids are falsified (research log 2026-09-15 §7). **The VoC routing proposition itself (H1-V1) is falsified at V1.** V0-passed only under the registered "one-shot or sequential" rule (3 of 5 budgets vs physics via the sequential pair; 4 of 5 vs adjoint; 4 of 5 at cost ratio 13); crossing frontiers — physics rule better at high budgets. Not supported with the constant-closure cheap model (no policy differs when the cheap model's error is not sparse). | V0 | same |
| H10 | inverse design | **V1-inconclusive — not testable as preregistered** (`v1_gunay_main`, `restoration/`): the Kf-loss damage leaves every evaluable instance within tolerance (10/10), so there is nothing to restore; exploratory NaT-loss variant (post-hoc, not a verdict): the routed search matches the uniform-fine search on every check (8/11 restored on fine A) at 26 % of its cost while the medium-only search's restorations fail fine A in 86 % of cases — but restorations found under formulation A transfer to formulation B in 1/11 for every method (research log 2026-09-15 §6.1) | V1 (precursor) | `results/v1_gunay_main/verdicts_extended.md` |
